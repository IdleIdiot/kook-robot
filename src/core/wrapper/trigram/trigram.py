...
from src.core.structure.trigram.yi_book import book


# 设置随机数种子，确保用户输入一样时，得到的随机数是一样的
def _set_random_seed(time_seed_str):
...
# 执行一轮变化
def _exec_one_transfer(cosmos_count):
...
def _gen_one_symbol(cosmos_count=49, transfer_index=0):
    cosmos_count = _exec_one_transfer(cosmos_count)
    transfer_index += 1
    if transfer_index >= 3:
        return cosmos_count
    else:
        return _gen_one_symbol(cosmos_count=cosmos_count, transfer_index=transfer_index)


# 获取六爻，组成本卦
def _gen_origin_hexagram():
    six_symbol_list = []
    for symbol_index in range(6):
        tmp_symbol = int(_gen_one_symbol() / 4)
        six_symbol_list.append(tmp_symbol)
    return six_symbol_list


# 根据本卦获取变卦
def _gen_support_hexagram(origin_hexagram):
    support_hexagram = []
...
# 分析变爻数及变爻下标列表
def _get_turn_symbol_count_and_index_list(origin_hexagram, support_hexagram):
    turn_symbol_count = 0
...
# 偶数为阴，奇数为阳
def _get_hexagram_dict_key(hexagram):
    hexagram_dict_key = ""
...
# 解卦
def _parser_hexagram(origin_hexagram, support_hexagram):
    main_indicate = ""
    support_indicate = ""
    turn_symbol_desc = ""
    origin_hexagram_dict_key = _get_hexagram_dict_key(origin_hexagram)
    support_hexagram_dict_key = _get_hexagram_dict_key(support_hexagram)
    turn_symbol_count, turn_symbol_index_list, not_turn_symbol_index_list = _get_turn_symbol_count_and_index_list(
        origin_hexagram, support_hexagram)
    # 六爻皆不变者，占本卦卦辞
...
def yi(wish, pray, yi_date):
    time_seed_str = wish + pray + yi_date
    # 设置随机数种子，确保当天同样的输出得到的结果是一样的
    _set_random_seed(time_seed_str)
    # 生成本卦
    origin_hexagram = _gen_origin_hexagram()
    # 计算变卦
    support_hexagram = _gen_support_hexagram(origin_hexagram)
    # 本卦对应的内容的key
    origin_hexagram_dict_key = _get_hexagram_dict_key(origin_hexagram)
    # 变卦对应的内容的key
    support_hexagram_dict_key = _get_hexagram_dict_key(support_hexagram)

    ori = "{}--{}--{}".format(origin_hexagram,
                              book[origin_hexagram_dict_key]['name'],
                              book[origin_hexagram_dict_key]['come_from'])
    shift = "{}--{}--{}".format(support_hexagram,
                                book[support_hexagram_dict_key]['name'],
                                book[support_hexagram_dict_key]['come_from'])

    # 解卦： 卦象  主解   辅解
    turn_symbol_desc, main_indicate, support_indicate = _parser_hexagram(origin_hexagram, support_hexagram)
    return ori, shift, turn_symbol_desc, main_indicate, support_indicate
...
if __name__ == "__main__":
    ...
    result = yi(user_wish, user_pray, today)
    print(result)
